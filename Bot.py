...
# ================== НАСТРОЙКИ ==================
BOT_TOKEN = "8904597240:AAGLimNlg8NlMAe1Qw1gWmNJYudWRjAoZhY"
...
def mark_person_keyboard(person_id: int, name: str):
    builder = InlineKeyboardBuilder()
...
async def choose_status(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        parse_mode="HTML",
        reply_markup=mark_person_keyboard(person_id, name)
    )
    await callback.answer()
...
async def mark_group_from_id(callback: CallbackQuery, group_id: int):
    """Вспомогательная функция — обновить список отметок"""
    today = date.today().isoformat()
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute(
            "SELECT id, full_name FROM people WHERE group_id = ? ORDER BY full_name",
...
async def show_stats(callback: CallbackQuery):
    ...
    if data == "stats_total":
        ...
        text = f"📊 <b>{GROUPS[group_id]}</b> на {today}\n\n"
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            if not people:
                ...
                for pid, name in people:
                    cursor = await db.execute(
                        "SELECT status, reason FROM marks WHERE day = ? AND person_id = ?",
                        (today, pid)
                    )
                    row = await cursor.fetchone()
                    if row is None:
                        not_marked.append(name)
                    elif row[0] == "present":
                        present_list.append(name)
                    else:
                        reason = row[1] or "без причины"
                        absent_list.append(f"{name} — {reason}")

                text += f"✅ Присутствует ({len(present_list)}):\n"
                text += "\n".join(f"• {n}" for n in present_list) or "—"
                text += f"\n\n❌ Отсутствует ({len(absent_list)}):\n"
                text += "\n".join(f"• {n}" for n in absent_list) or "—"
                if not_marked:
                    text += f"\n\n⬜ Ещё не отмечены ({len(not_marked)}):\n"
                    text += "\n".join(f"• {n}" for n in not_marked)
